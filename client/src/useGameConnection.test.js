...
describe('useGameConnection', () => {
  ...
  describe('gameOver event', () => {
    it('preserves session and chat so a post-game reconnect works', () => {
  ...
  describe('returnToLobby', () => {
    it('emits the returnToLobby event', () => {
      const { result } = renderHook(() => useGameConnection());
      act(() => {
        result.current.returnToLobby();
      });
      expect(mockSocket.emit).toHaveBeenCalledWith('returnToLobby', undefined);
    });
  });

  describe('playerLeftPostGame', () => {
